...
export const DataTable: FC = () => {
  const getColumnSearchProps = (): TableColumnType<Data> => ({
    filterDropdown: (props) => <FilterForm {...props} />,
  ...
  const columns: TableColumnType<Data>[] = useMemo(
    () => [
      {
        title: "شماره تراکنش",
        align: "center",
      ...
      },
      {
        ...
        align: "center",
        render: (value) => (
          <div className="ltr-dir">{formatCardNumber(value)}</div>
        ),
  ...
  return (
    <Table
      ...
      pagination={false}
      summary={(data) => (
        <div className="p-3">{"تعداد نتایج: " + data.length}</div>
      )}
    />
  );
};
